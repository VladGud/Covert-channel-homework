...
def calculate_bandwidth_with_errors(n):
	t = (n + 1) / 2
	h = 0
...
def calculate_bandwidth_with_probability(n):
	t = (n + 1) / 2
	h = 0
...
calculate_fn = [calculate_bandwidth, calculate_bandwidth_with_errors, calculate_bandwidth_with_probability]
...
for fn in calculate_fn:
	...
	for n in range(1, randrange(20, 40)):
		...
		n_x.append(n)
	plt.suptitle('График зависимости пропускной способности от числа пакетов')
	plt.xlabel('Число пакетов')
	plt.ylabel('Пропускная способность')
	plt.xticks(n_x)
	plt.plot(n_x, bandwidths)
	plt.scatter(n_find, bandwidth_max, color='green', marker='o')
	plt.show()
	plt.clf()

print("Максимальная пропускная способность = ", bandwidth_max)
print("Число пакетов = ", n_find)
print(bandwidths)